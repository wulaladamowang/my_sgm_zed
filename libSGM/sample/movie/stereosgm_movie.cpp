...
int main(int argc, char* argv[])
{
	...
	cv::Size siz = cv::Size(I1.cols/2, I1.rows/2);
	cv::resize(I1, I1, siz);
	cv::resize(I2, I2, siz);
	...
	for (int frame_no = first_frame;; frame_no++) {
		...
		disparity.convertTo(disparity_8u, CV_8U, 255. / 64);//disp_size);
		...
		std::cout << "--------------------" << std::endl;
		std::cout << "R: " << disparity_color.ptr<cv::Vec3b>(height/2, width/2)
				  << "G: " << disparity_color.ptr<cv::Vec3b>(height/2, width/2) 
				  << "B: " << disparity_color.ptr<cv::Vec3b>(height/2, width/2)[2] 
		 		  << std::endl;
		cv::circle(disparity_color, cv::Point(width/2, height/2), 10, cv::Scalar(255, 255, 255));

		cv::imshow("left image", I1);
		cv::imshow("disparity 8u", disparity);
