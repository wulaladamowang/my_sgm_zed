...
int main(int argc, char* argv[])
{
	...
	int a;
    sscanf(argv[6], "%d", &a);
    cv::Size siz = cv::Size(I1.cols/a, I1.rows/a);
	cv::resize(I1, I1, siz);
	cv::resize(I2, I2, siz);
	...
	for (int frame_no = first_frame;; frame_no++) {
		...
		disparity_32f.convertTo(disparity_8u, CV_8U, 255. / 64);//disp_size);
		...
		// std::cout << "--------------------" << std::endl;
		// std::cout << "RGB: " << disparity_color.ptr<cv::Vec3b>(height/2, width/2)[0] 
		//  		  << std::endl;
		std::cout << disparity_32f.ptr<float>(height/2)[width/2] << std::endl;
