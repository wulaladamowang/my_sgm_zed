...
int main(int argc, char* argv[]) {	
	...
	if (argc > 1) initParameters.input.setFromSVOFile(argv[1]);
	...
	sl::Mat d_zed_image_depth(zed.getCameraInformation().camera_resolution, sl::MAT_TYPE::U8_C4);
	cv::Mat depth_image_ocv = cv::Mat(d_zed_image_depth.getHeight(), d_zed_image_depth.getWidth(), CV_8UC4, d_zed_image_depth.getPtr<sl::uchar1>(sl::MEM::CPU));	
	...
	cv::Mat disparity_8u, disparity_color, depth_image_ocv_8u, depth_image_ocv_color;
	...
	while (1) {
		if (zed.grab() == sl::ERROR_CODE::SUCCESS) {
			...
			zed.retrieveImage(d_zed_image_depth, sl::VIEW::DEPTH, sl::MEM::CPU);
		} else continue;
		...
		depth_image_ocv.convertTo(depth_image_ocv_8u, CV_8U, 255. / disp_size);

		// cv::applyColorMap(disparity_8u, disparity_color, cv::COLORMAP_JET);
		// cv::applyColorMap(depth_image_ocv_8u, depth_image_ocv_color, cv::COLORMAP_JET);
		...
		cv::bitwise_not(disparity_8u, disparity_8u);
		cv::imshow("disparity", disparity_8u);
		cv::imshow("depth", depth_image_ocv_8u);
